...
import {
    ...
    Button, Avatar, Menu, MenuItem, Badge, Divider
} from '@mui/material';
...
function MainLayout() {
    ...
    const user = JSON.parse(localStorage.getItem('user') || '{"prenom":"Admin","nom":"User"}');
    ...
    const menuItems = [
        ...
        { text: '📅 Prise RDV', icon: <EventNoteIcon />, path: '/patient-portal' },
        { text: '🔬 Résultats', icon: <ScienceIcon />, path: '/exam-results' },
    ...
    const drawerContent = (
        <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: '#1a1a2e', color: 'white' }}>
            ...
            <Box sx={{ p: 2, borderTop: '1px solid rgba(255,255,255,0.1)' }}>
                <Button fullWidth variant="outlined" startIcon={<LogoutIcon />} onClick={handleLogout} sx={{ color: '#ff6b6b', borderColor: '#ff6b6b' }}>
                    Déconnexion
                </Button>
    ...
    return (
        <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: '#f5f5f5' }}>
            <AppBar position="fixed" sx={{ zIndex: 1200, bgcolor: 'white', color: '#333', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
                <Toolbar>
                    <IconButton color="inherit" edge="start" onClick={() => setMobileOpen(true)} sx={{ display: { sm: 'none' }, mr: 2 }}>
                        <MenuIcon />
                    </IconButton>
                    <Typography variant="h6" sx={{ flexGrow: 1, color: '#00bcd4', fontWeight: 'bold' }}>Tableau de bord</Typography>
                    <IconButton color="inherit"><Badge badgeContent={3} color="error"><NotificationsIcon /></Badge></IconButton>
                    <Button color="inherit" onClick={(e) => setAnchorEl(e.currentTarget)} startIcon={<Avatar sx={{ width: 32, height: 32, bgcolor: '#00bcd4' }}>{user.prenom?.charAt(0)}{user.nom?.charAt(0)}</Avatar>}>
            ...
            <Box component="nav" sx={{ width: { sm: drawerWidth }, flexShrink: { sm: 0 } }}>
                ...
                <Drawer variant="permanent" sx={{ display: { xs: 'none', sm: 'block' }, '& .MuiDrawer-paper': { width: drawerWidth, boxSizing: 'border-box', borderRight: 'none' } }} open>
                    {drawerContent}
                </Drawer>
            </Box>
